...
def standardize_ms_forms_dataframe(raw_df):
    ...
    text_mapping = {
        ...
        "نقاط القوة في أدائي العام": ["نقاط القوة في أدائي العام", "نقاط القوة في ادائي العام", "نقاط القوة بأدائي العام", "نقاط القوة في الأداء العام", "قوتي في أدائي العام"],
        "نقاط الضعف التي تحتاج إلى تطوير": ["نقاط الضعف التي تحتاج إلى تطوير", "نقاط الضعف التي تحتاج الى تطوير", "نقاط الضعف في أدائي العام", "نقاط الضعف في ادائي العام", "الجوانب التي تحتاج إلى تطوير في أدائي", "الجوانب التي تحتاج الى تطوير في ادائي", "الجوانب التي تحتاج إلى تطوير في أدائي العام", "جوانب الضعف في أدائي العام", "ما الجوانب التي تحتاج إلى تطوير"],
        "الدعم المطلوب من زيارات القيادة الوسطى": ["ما نوع الدعم الذي أحتاجه من القيادة الوسطى لتطوير أدائي", "ما نوع الدعم الذي احتاجه من القيادة الوسطى لتطوير ادائي", "الدعم المطلوب من زيارات القيادة الوسطى", "الدعم المطلوب", "نوع الدعم", "الدعم الذي أحتاجه", "الدعم الذي احتاجه"],
        "مقترحاتي لتطوير أدائي": ["مقترحاتي لتطوير أدائي في المواقف التعليمية", "مقترحاتي لتطوير ادائي في المواقف التعليمية", "مقترحاتي لتطوير أدائي", "مقترحاتي لتطوير ادائي", "مقترحات لتطوير أدائي", "مقترحات لتطوير ادائي", "ما مقترحاتي لتطوير أدائي", "مقترحاتي"],
    ...
    # REAL5: بعض أعمدة Microsoft Forms تأتي باسم السؤال الطويل أو بصيغة مختلفة،
    # لذلك نستخدم بحثاً مرناً إذا لم نجد العمود بالاسم المطابق.
    def _norm_col_text(s):
        return normalize_text(str(s).replace("\xa0", " ").replace("ى", "ي").replace("ؤ", "و").replace("ئ", "ي")).lower().strip()

    def _fuzzy_fill_text_column(out_col, patterns):
        try:
            current = out[out_col] if out_col in out.columns else pd.Series([pd.NA] * len(df), index=df.index)
            has_current = current.dropna().astype(str).str.strip().ne("").any()
            if has_current:
                return
            matches = []
            norm_patterns = [_norm_col_text(p) for p in patterns if str(p).strip()]
            for col in df.columns:
                nc = _norm_col_text(col)
                if any(p and (p in nc or nc in p) for p in norm_patterns):
                    matches.append(col)
            if matches:
                out[out_col] = combine_first_non_empty(df, matches)
        except Exception:
            pass

    fuzzy_text_patterns = {
        "نقاط القوة في أدائي العام": [
            "نقاط القوة في أدائي العام", "نقاط القوة في ادائي العام", "نقاط القوة في الأداء العام",
            "ما نقاط القوة في أدائي", "نقاط القوة لدي",
        ],
        "نقاط الضعف التي تحتاج إلى تطوير": [
            "نقاط الضعف التي تحتاج إلى تطوير", "نقاط الضعف التي تحتاج الى تطوير",
            "نقاط الضعف في أدائي العام", "الجوانب التي تحتاج إلى تطوير في أدائي",
            "الجوانب التي تحتاج إلى تطوير", "جوانب الضعف",
        ],
        "الدعم المطلوب من زيارات القيادة الوسطى": [
            "ما نوع الدعم الذي أحتاجه من القيادة الوسطى لتطوير أدائي",
            "ما نوع الدعم الذي احتاجه من القيادة الوسطى لتطوير ادائي",
            "الدعم المطلوب من زيارات القيادة الوسطى", "الدعم المطلوب", "نوع الدعم",
            "الدعم الذي أحتاجه", "الدعم الذي احتاجه",
        ],
        "مقترحاتي لتطوير أدائي": [
            "مقترحاتي لتطوير أدائي في المواقف التعليمية", "مقترحاتي لتطوير ادائي في المواقف التعليمية",
            "مقترحاتي لتطوير أدائي", "مقترحاتي لتطوير ادائي", "مقترحات لتطوير أدائي",
            "ما مقترحاتي لتطوير أدائي", "مقترحاتي",
        ],
    }
    for _out_col, _patterns in fuzzy_text_patterns.items():
        _fuzzy_fill_text_column(_out_col, _patterns)
...
def show_analysis(df, allowed_dept):
    ...
    if "الفصل الدراسي" in df.columns and df["الفصل الدراسي"].nunique() >= 2:
        ...
        if len(sems_available) >= 2:
            ...
            st.caption("تم حساب المقارنة بنفس طريقة النسبة العامة: من جميع السجلات داخل كل فصل، مع تطبيق نفس الفلاتر المختارة ما عدا فلتر الفصل الدراسي.")

            # REAL7: مقارنة الفصلين يجب أن تطابق النسبة العامة، لذلك نحسبها من كل سجلات الفصل وليس من متوسط الأشهر.
